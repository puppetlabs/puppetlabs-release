component 'archive_repo_definition' do |pkg, settings, platform|
  pkg.version '2019.05.15'

  if platform.is_deb?
    pkg.url 'file://files/puppet-archives.list.txt'
    pkg.md5sum '53d2e1455bab67b4a49a5d0969ebbb95'
    pkg.install_configfile 'puppet-archives.list.txt', '/etc/apt/sources.list.d/puppet-archives.list'
  if platform.name =~ /^ubuntu-16.10/
    ...
    pkg.install_configfile 'puppetlabs-puppet-agent-pin-1000.txt', '/etc/apt/preferences.d/puppetlabs-archives-puppet-agent-pin-1000'
  end
    pkg.install do
      "sed -i 's|__CODENAME__|#{platform.codename}|g' /etc/apt/sources.list.d/puppet-archives.list"
    end
  else
    ...
    pkg.url 'file://files/puppet-archives.repo.txt'
    pkg.md5sum 'c5b79dc2f8a13d710a17e5f3ca502376'
    pkg.install_configfile 'puppet-archives.repo.txt', "#{repo_path}/puppet-archives.repo"

    install_hash = ["sed -i -e 's|__OS_NAME__|#{platform.os_name}|g' -e 's|__OS_VERSION__|#{platform.os_version}|g' #{repo_path}/puppet-archives.repo"]
